...
int main() {
	...
	strncpy(msg, "broadcast message", strlen("broadcast message"));
	...
	rtn = setsockopt(sd_srv, SOL_SOCKET, SO_BROADCAST, (char *)&on, sizeof(on));
	if (rtn == -1) {
		perror("Server: setsockopt(SO_BROADCAST)");
